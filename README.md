...
rather than reckless. Authenticates via a Claude Pro/Max subscription OAuth
token, not a pay-per-token Anthropic API key: run `claude setup-token`
locally (opens a browser OAuth flow, prints a long-lived token), then add it
as a repository secret named `CLAUDE_CODE_OAUTH_TOKEN` (Settings -> Secrets
and variables -> Actions -> Repository secrets - not an environment secret,
which needs `environment:` added to every job that reads it, including
scheduled ones). Also needs `id-token: write` in the workflow's permissions
- `claude-code-action` fetches a GitHub OIDC token as part of its own setup
regardless of auth method, and hard-fails without it.

Both the id-token requirement and an earlier attempt with a raw
`anthropic_api_key` (which got all the way to a real model call before
failing on `billing_error: Credit balance is too low` - a $0 Anthropic
Console credit balance, not a config problem) were found by actually
